...
| [submodule-signature-audit.md](submodule-signature-audit.md) | Why submodule-backed models break when **any** `submodules/` pin moves (they re-implement forward and call submodule internals directly), which submodule backs which model, the step-by-step audit to run after every submodule update, and the automated ComfyUI guard `tools/check_comfy_signatures.py`. Includes the 2026-06-29 ComfyUI audit results. |
